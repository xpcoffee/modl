...
export const SLOW_STEP_MS = 1000;
...
/**
 * Repeats a step while an input is held down: one step per second for the
 * first three seconds, then three per second. Slow first, so a reader who
 * overshot by one has time to let go; fast after, so a long list does not
 * take a minute to cross (issue #66).
 *
