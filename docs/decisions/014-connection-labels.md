...
**A label is drawn while its junction or its line is being read.** Hovering or selecting the decision shows every answer it gives; selecting a connection shows the answers written against it, which is one per end when it runs between two decisions. Nothing shows otherwise, so a board of decisions is not a wall of text. Labels sit on the line, out from the junction they belong to, which is what makes a label at each end of one line readable.
