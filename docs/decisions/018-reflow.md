...
**Labels are obstacles, moved only through their endpoints.** On a dense board most of the ink is labels, and the midpoint of a long line lands wherever the boxes between its ends happen to sit. Each drawn label therefore enters the solver as a rectangle riding the midpoint of its two endpoints: a solid box covering it eases away while the endpoints ease the other way (12px of clearance), and two labels stacked on each other unstack vertically through their endpoints (8px). Expanded containers are outlines, so a label may cross one; the solid members inside ride their container as satellites, and a label clearing a member pushes the container. Two labels whose movable endpoints are the same boxes cannot separate this way and are left to the renderer, which fans parallel lines apart. Every push a frozen box swallows counts as no progress, so an unsatisfiable constraint cannot spin the solver.

**Scopes go deepest-first, then a world-aware pass, restarted until still.** Members re-space inside their container, and the container grows (position and `expanded` size) only when a member would poke out of it; a box the reader sized on purpose is otherwise left alone. Each expanded group then takes part in its parent scope as one box of its container size. The first pass over a group cannot see where lines leaving it go, so once the root has settled, each group's members solve again with the rest of the board frozen in place; that pass is what spreads labels riding the same pair of containers, which container moves shift identically. A cycle of these passes can conflict (a member push grows its container, the root pushes containers apart, the labels land somewhere new), so the cycle stops at the first round that moved further than its predecessor and puts that round back, and the whole pipeline restarts until a run holds still. A still run is also what makes the plan a fixed point: pressing the button on its own output computes nothing. A collapsed group moves as one box with every descendant carried along, and a connection's hand-placed bends shift by the average of how far its endpoints moved, so a drawn route keeps its shape. A pinned comment card takes part in the scope of the innermost expanded container drawn around it, or at the root, with the same clearance as any box. Hidden elements draw muted but stay on the board, so they keep their room like anything else.
