...
**A drag action can instead run begin+end.** Box select and duplicate carry a mode: `drag` holds the binding through a drag as before; `begin-end` opens a run on one press (key or mouse, since nothing needs holding), grows it with pointer movement, settles it on the next press, and abandons it on the cancel binding. Begin+end box select draws its own rectangle (`BoxSelectPreview`), because React Flow only draws one for a held drag; both settle through the same `settleBox`/`settleCopies` code as the drag forms.
...
**Captures are typed by action and mode.** A keyboard action records the next key press with its modifiers. Pan records a button and drops modifiers, because React Flow pans by button alone. Duplicate in drag mode refuses a bare left button, which would swallow the plain drag that moves elements. During a mouse capture, a bare left click on one of the panel's own controls reads as using the panel and disarms; from anywhere else it binds, so the bare left drag stays assignable. The panel dismisses on a backdrop click only when the press also began on the backdrop, so a drag released outside stays open.
