...
**A note carries element tags, and the filter reads them both ways.** `tags` has the same `{key: [values]}` shape as element tags, so one filter grammar covers both. A committed tag filter hides notes that do not carry the tag, which is how a reader dials the board to one situation, and an element matches a tag filter when one of its notes carries the tag, so filtering to a context lights up the elements that context touches.
...
This is a proof of concept. Feedback on issue #83 decides whether the name, the placement inside `model`, or the always-visible card presentation survive; a reversal before acceptance costs one migration.
