...
**A comment needs at least one target, and dies with its last one.** Detached discussion is noise with no way back to what it was about. Deleting an element strips it from every comment's targets, cascades included, and a comment left with nothing to discuss is deleted, the same rule a connection follows when it loses an endpoint. The schema enforces `targets.min(1)`, and `set-comment-targets` refuses an empty list.
