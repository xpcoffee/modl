...
**Going to an element pans to its visible anchor and selects it.** Same shape as pan-to-relation: a `set-view` command and a `set-selection`, so the camera follows the bus and a trace records where the reader went. An element inside a collapsed group has no place on the board of its own, so the camera goes to the group standing in for it rather than to coordinates nothing is drawn at.
