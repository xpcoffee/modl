...
Note ids share the selection's id space with elements and comments, so selecting a note is the same gesture as selecting a box, and it highlights the elements the note describes. A note the reader has arranged on the board keeps a pin in `layout` under its own id, `{x, y, width, height}`, written by `move-note` (or `create-note` with a `position`) and deleted with the note. A note without a pin gets a derived spot beside its targets that covers no element, and a pin holds only while the board draws its saved geometry: under focus mode's compacted overlay every card derives from where its targets are drawn, and the pin returns with the saved layout. Reflow and compact carry a pinned note card with the scope that holds it, the way they carry a pinned comment card. See [decision 032](decisions/032-note-card-placement.md).
