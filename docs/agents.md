...
Context that spans elements goes into `model.notes` rather than into one element's `description`: a note carries `text`, `targets` (element ids; empty means the whole document), and `tags` in the same shape element tags take, so a producer can label a note `context=refunds` and a filter finds both the note and the elements it describes. The commands are `create-note`, `set-note-text`, `set-note-targets`, `set-note-tag`, `remove-note-tag`, `move-note`, and `delete-note`. Leave placement out and the board derives a spot beside the note's targets that covers no element; `create-note` also takes an optional `position` that pins the card there in one command, and `modl check` reports a pinned note covering an element (see [decision 032](decisions/032-note-card-placement.md)).
